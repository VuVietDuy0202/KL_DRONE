...
//         float out = kp * error + integrator + kd * derivative;
//         if (out > outMax)
//             out = outMax;
//         if (out < outMin)
//             out = outMin;
...
// PID pidRoll(1.4f, 0.675f, 0.085f, -250, 250, 100);
// PID pidPitch(1.64f, 0.0f, 0.0f, -250, 250, 100);
// PID pidYaw(0.05f, 0.05f, 0.0005f, -300, 300, 150);

// // PID Angle (outer loop)
// float KpAngleRoll = 4.0f;
// float KiAngleRoll = 0.05f; // Thay đổi giá trị này để chỉnh I angle roll
// float KpAnglePitch = 0.0f;
// float KiAnglePitch = 0.0f; // Thay đổi giá trị này để chỉnh I angle pitch

// float integralAngleRoll = 0.0f;
// float integralAnglePitch = 0.0f;
// const float iAngleLimit = 100.0f;
...
//     // --- PID Angle (outer loop) ---
//     float errorAngleRoll = targetRollDeg - rollDeg;
//     float errorAnglePitch = targetPitchDeg - pitchDeg;

//     integralAngleRoll += errorAngleRoll * KiAngleRoll * dt;
//     integralAnglePitch += errorAnglePitch * KiAnglePitch * dt;
//     // Giới hạn tích phân tránh windup
//     if (integralAngleRoll > iAngleLimit)
//         integralAngleRoll = iAngleLimit;
//     if (integralAngleRoll < -iAngleLimit)
//         integralAngleRoll = -iAngleLimit;
//     if (integralAnglePitch > iAngleLimit)
//         integralAnglePitch = iAngleLimit;
//     if (integralAnglePitch < -iAngleLimit)
//         integralAnglePitch = -iAngleLimit;

//     float spRollRate = KpAngleRoll * errorAngleRoll + integralAngleRoll;
//     float spPitchRate = KpAnglePitch * errorAnglePitch + integralAnglePitch;
...
//         pidPitch.reset();
//         pidYaw.reset();
//         integralAngleRoll = 0.0f;
//         integralAnglePitch = 0.0f;
//     }
//     else
//     {
